...
export class Model {
  ...
  async list(queryData: IQueryData): Promise<{ page: object, data: IModel[] }> {
    ...
    const setSize: number = (pageSize > config.maxPageSizeLimit) ? config.maxPageSizeLimit : pageSize
    const sortBy: ISort = (sorter && sorter !== config.sortTypes.date) ? { [config.sortTypes[sorter]]: 1 } : { createdAt: -1 }
    ...
    const result: IModel[] = await this.model.find(query).limit(setSize).skip((current - 1) * setSize).sort(sortBy)
    return {
  ...
  async removePatch(modelIds: string[]): Promise<{ ok?: number, n?: number } & { deletedCount?: number }> {
