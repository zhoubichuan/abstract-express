...
declare module './mongo_base' {
  interface Model {
    ...
    greetings: (systemtagId: string) => Promise<string>
    findByVideo: (video: string) => Promise<ISystemtag>
  }
}

/** Get Model Mane */
Model.prototype.greetings = async function(systemtagId: string): Promise<string> {
  const sample: ISystemtag | null = await this.model.findById(systemtagId)
  console.log(sample)
...
/** Find Model By Age */
Model.prototype.findByVideo = async function(video: string): Promise<ISystemtag> {
  const sample: ISystemtag | null = await this.model.findOne({ video })
  if(!sample) throw new Errors.NotFound(MESSAGES.MODEL_NOT_FOUND)
  return sample
}
...
// -----------------------------------------------------------------------------------
export interface ISystemtag extends IModel {
  name : string
  nameEn:string
  descript:string
  descriptEn:string
...
export interface ISystemtagUpdate extends IModelUpdate {
  name? : ISystemtag['name']
  video?  : ISystemtag['video']
}
...
baseModel.model = mongoose.model<ISystemtag>('systemtag', baseModel.schema)
