...
declare module './mongo_base' {
  interface Model {
    ...
    greetings: (productId: string) => Promise<string>
    findByVideo: (video: string) => Promise<IProduct>
  }
}

/** Get Model Mane */
Model.prototype.greetings = async function(productId: string): Promise<string> {
  const sample: IProduct | null = await this.model.findById(productId)
  console.log(sample)
...
/** Find Model By Age */
Model.prototype.findByVideo = async function(video: string): Promise<IProduct> {
  const sample: IProduct | null = await this.model.findOne({ video })
  if(!sample) throw new Errors.NotFound(MESSAGES.MODEL_NOT_FOUND)
  return sample
}
...
export interface IProductUpdate extends IModelUpdate {
  name? : IProduct['name']
...
baseModel.model = mongoose.model<IProduct>('product', baseModel.schema)
